...
import com.dwill.gamestore.repository.user.UserRepo;
...
public class UserService implements UserDetailsService {
    ...
    public String signUpUser(AppUser appUser) {
        ...
        if (userExists){
            if (userRepo.findTopByName(appUser.getName()).isPresent())
            {
                AppUser oldUser = userRepo.findByEmail(appUser.getEmail()).get();
                if (!oldUser.getEnabled()){
                    {
                        ...
                        emailSender.send(oldUser.getEmail(), buildEmail(oldUser.getName(), link));
                        throw new IllegalStateException("Email in system, not confirmed.");
                    }
                }
            }
